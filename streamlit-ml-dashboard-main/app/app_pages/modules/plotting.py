...
def save_predictions_history(predictions, prediction_date, model_dir='models'):
    """
    Guarda las predicciones realizadas en una fecha específica.
    Actualiza el archivo de historial de predicciones.
    
    Args:
        predictions: DataFrame con columnas [timestamp, series_id, pred]
        prediction_date: Fecha en que se hicieron las predicciones (datetime o string)
        model_dir: Directorio donde guardar el historial
    """
    predictions_history_path = os.path.join(model_dir, 'predictions_history.joblib')
    
    # Añadir columna con la fecha de predicción
    predictions_to_save = predictions.copy()
    predictions_to_save['prediction_date'] = pd.to_datetime(prediction_date)
    
    # Cargar historial existente o crear uno nuevo
    if os.path.exists(predictions_history_path):
        history = joblib.load(predictions_history_path)
        # Concatenar con nuevas predicciones
        history = pd.concat([history, predictions_to_save], ignore_index=True)
    else:
        history = predictions_to_save
    
    # Guardar historial actualizado
    joblib.dump(history, predictions_history_path)
    
    return history


def load_predictions_history(model_dir='models'):
    """
    Carga el historial completo de predicciones.
    
    Returns:
        DataFrame con columnas [timestamp, series_id, pred, prediction_date]
        o None si no existe el archivo
    """
    predictions_history_path = os.path.join(model_dir, 'predictions_history.joblib')
    
    if os.path.exists(predictions_history_path):
        return joblib.load(predictions_history_path)
    else:
        return None


def get_relevant_past_predictions(predictions_history, model_start_date, today):
    """
    Filtra las predicciones históricas relevantes para la visualización.
    Mantiene solo la predicción más antigua para cada timestamp/series_id,
    es decir, la primera vez que se predijo ese punto.
    
    Args:
        predictions_history: DataFrame con historial completo
        model_start_date: Fecha de inicio del modelo
        today: Fecha actual
        
    Returns:
        DataFrame con predicciones filtradas
    """
    if predictions_history is None or len(predictions_history) == 0:
        return pd.DataFrame()
    
    # Filtrar predicciones hechas antes de "hoy" para timestamps hasta "hoy"
    relevant = predictions_history[
        (predictions_history['prediction_date'] < pd.Timestamp(today)) &
        (predictions_history['timestamp'] >= pd.Timestamp(model_start_date)) &
        (predictions_history['timestamp'] <= pd.Timestamp(today))
    ].copy()
    
    # Para cada timestamp/series_id, quedarnos con la predicción más antigua
    # (la primera vez que se predijo ese punto)
    if len(relevant) > 0:
        relevant = relevant.sort_values('prediction_date')
        relevant = relevant.groupby(['timestamp', 'series_id']).first().reset_index()
    
    return relevant


def clear_predictions_history(model_dir='models'):
    """
    Elimina el historial de predicciones (útil para resetear).
    """
    predictions_history_path = os.path.join(model_dir, 'predictions_history.joblib')
    if os.path.exists(predictions_history_path):
        os.remove(predictions_history_path)
...
def create_forecast_plot_with_history(series_full, predictions_for, 
                                     predictions_history=None,
                                     selected_series_id=None, 
                                     model_start_date='2025-07-30', 
                                     today='2025-08-27'):
    """
    Crea una gráfica que muestra:
    - Histórico real completo (línea azul sólida)
    - Predicciones históricas guardadas (línea naranja punteada)
    - Cutoff vertical (línea roja) representando "hoy"
    - Predicciones futuras actuales (línea verde desde hoy)
    
    Args:
        series_full: DataFrame con datos históricos (timestamp, series_id, pred/ADD)
        predictions_for: DataFrame con predicciones actuales (timestamp, series_id, pred)
        predictions_history: DataFrame con historial de predicciones
        selected_series_id: ID de la serie a visualizar
        model_start_date: Fecha en que el modelo empezó a predecir
        today: Fecha "actual"
    """
    import plotly.graph_objects as go
    ...
    # Determinar la columna de demanda en series_full
    demand_col = 'pred' if 'pred' in series_full.columns else 'ADD'
    
    # Si no se especifica series_id, usar el primero disponible
    if selected_series_id is None:
        selected_series_id = series_full['series_id'].iloc[0]
    
    # 1. Datos históricos reales
    historical = series_full[series_full['series_id'] == selected_series_id].copy()
    ...
    # 2. Predicciones futuras (desde hoy hacia adelante)
    future_preds = predictions_for[predictions_for['series_id'] == selected_series_id].copy()
    future_preds = future_preds.sort_values('timestamp')
    future_preds['timestamp'] = pd.to_datetime(future_preds['timestamp'])
    future_preds = future_preds[future_preds['timestamp'] > today]
    
    # 3. Predicciones históricas (si existen)
    past_preds = pd.DataFrame()
    if predictions_history is not None and len(predictions_history) > 0:
        past_preds = get_relevant_past_predictions(
            predictions_history, 
            model_start_date, 
            today
        )
        past_preds = past_preds[past_preds['series_id'] == selected_series_id].copy()
        past_preds = past_preds.sort_values('timestamp')
    
    # Crear la figura
    fig = go.Figure()
    
    # Traza 1: Histórico real completo
    if len(historical_until_today) > 0:
        fig.add_trace(go.Scatter(
            x=historical_until_today['timestamp'],
            y=historical_until_today[demand_col],
            mode='lines+markers',
            name='Actual Data',
    ...
    # Traza 2: Predicciones históricas
    if len(past_preds) > 0:
        fig.add_trace(go.Scatter(
            x=past_preds['timestamp'],
            y=past_preds['pred'],
            mode='lines+markers',
            name='Past Predictions',
            line=dict(color='#ff7f0e', width=2, dash='dot'),
            marker=dict(size=6, symbol='diamond'),
            hovertemplate='<b>Past Prediction</b><br>Date: %{x}<br>Value: %{y:.4f}<extra></extra>'
        ))
    
    # Traza 3: Predicciones futuras
    if len(future_preds) > 0:
        fig.add_trace(go.Scatter(
            x=future_preds['timestamp'],
            y=future_preds['pred'],
            mode='lines+markers',
            name='Future Predictions',
            line=dict(color='#2ca02c', width=2.5),
            marker=dict(size=8),
            hovertemplate='<b>Future Prediction</b><br>Date: %{x}<br>Value: %{y:.4f}<extra></extra>'
        ))
    ...
    fig.add_vline(
        x=model_start_date.timestamp() * 1000,
        line_dash="dash",
        line_color="purple",
        line_width=1.5,
        annotation_text="Model Start",
        annotation_position="top"
    )
    
    # Layout
    fig.update_layout(
        title=f'Forecast Timeline - Series: {selected_series_id}',
        xaxis_title='Date',
        yaxis_title='Demand',
        hovermode='x unified',
        height=500,
